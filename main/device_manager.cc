    ...
    if (err == ESP_OK) {
        ...
        if (err == ESP_OK && token_len > 0) {
            ...
            if (err == ESP_OK) {
                ...
                is_bound_ = true;
                ESP_LOGI(TAG, "Loaded device token from NVS (length: %d)", (int)(token_len - 1));
            }
            delete[] token_buf;
        }
...
bool DeviceManager::BindDevice(const std::string& binding_code, const std::string& device_name) {
    ...
    cJSON_AddStringToObject(request, "device_name",
        device_name.empty() ? "ESP32音乐播放器" : device_name.c_str());
    ...
    if (!http->Open("POST", DEVICE_BIND_API_URL)) {
...
bool DeviceManager::VerifyDevice() {
    ...
    if (!http->Open("GET", DEVICE_VERIFY_API_URL)) {
...
std::string DeviceManager::GetFavorites() {
    ...
    if (!http->Open("GET", FAVORITE_LIST_API_URL)) {
...
std::string DeviceManager::GetUserPlaylists() {
    ...
    if (!http->Open("GET", PLAYLIST_LIST_API_URL)) {
...
bool DeviceManager::TryFetchTokenFromServer() {
    ...
    std::string sync_url = std::string(MUSIC_SERVER_URL) + "/api/esp32/sync";
